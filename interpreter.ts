...
class BinaryExpr<T extends Expression, R extends Literal> implements Expression {
    constructor(
        public operator: string,
...
enum TokenCode {
    PUSH_VALUE,
    DECLARE_VAR, ASSIGN_VAR, GET_VAR_VALUE,
    PRINT,

    ADD, SUBTR, MULT, DIV, MOD,
    EQUAL, N_EQUAL, LOWER, LOWER_EQ, GREATER, GREATER_EQ,
    AND, OR,
    NOT,
...
class Executer {
    ...
    Partial<Record<TokenCode, (a: any, b: any) => any>> = {
    ...
    Partial<Record<TokenCode, (op: any) => any>> = {
        [TokenCode.NOT]: (op) => !op
    };
